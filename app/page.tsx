...
export default function Page() {
  ...
  const ashesWon =
    !!state.ashes?.complete && state.ashes.englandWins > state.ashes.australiaWins;
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-4 flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="text-emerald-200">
              {state.club.name} · {DIVISIONS[state.club.division].name} · Bank{" "}
              {money(state.club.bankBalance)}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              className="btn2"
              onClick={() => setState(youthPull(state))}
            ...
            <button
              className="btn2"
              onClick={() => navigator.clipboard.writeText(exportSave(state))}
            >
              Copy Save JSON
            </button>
        ...
        <nav className="mb-5 flex flex-wrap gap-2">
          {tabs.map((t) => (
            <button
              key={t}
              className={tab === t ? "btn" : "btn2"}
              onClick={() => setTab(t)}
            >
              {t}
              {t === "Inbox"
                ? ` (${state.inbox.filter((e) => !e.resolved).length})`
                : ""}
            </button>
          ))}
        </nav>
        ...
        {state.phase === "complete" && (
          <div className="card mb-5 p-6">
            ...
            <p className="text-emerald-200">
              ...
              Final Level {DIVISIONS[state.club.division].name}
            </p>
          </div>
        )}

        {tab === "Dashboard" && (
          <Dashboard state={state} onAdvance={() => setState(advanceWeek(state))} />
        )}
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-2">
          ...
          <div className="card p-4">
            ...
            <button
              className="btn mt-2"
              onClick={() => {
                const imported = importSave(importBox);
